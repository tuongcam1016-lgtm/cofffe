...
(async () => {
  ...
  await page.waitForSelector('.product-video-modal.is-open iframe[src*="youtube"]', { timeout: 5000 });
  const iframeSrc = await page.locator("[data-video-frame]").getAttribute("src");
  await page.screenshot({ path: "C:/tmp/tng-video-autoplay.png", fullPage: false });
  await browser.close();
  ...
  console.log(JSON.stringify({
    ...
    consoleErrors,
    screenshot: "C:/tmp/tng-video-autoplay.png"
  }, null, 2));
})().catch((error) => {
  console.error(error);
  process.exit(1);
